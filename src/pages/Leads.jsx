...
const Leads = () => {
  ...
  // NEW: Reset state when user returns to the tab
  useEffect(() => {
    const handleFocus = () => {
  ...
  const handleJoinClick = () => {
    ...
    // The delay gives the pixel time to fire before the page changes
    setTimeout(() => {
      ...
      <div className="max-w-md w-full bg-white rounded-3xl shadow-2xl overflow-hidden border border-white">
        ...
        <div className="p-8">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-black text-slate-800 tracking-tight uppercase">Color Prediction</h1>
            <p className="text-slate-500 text-sm mt-1 font-medium">Verified Community Access</p>
          </div>

          <div className="space-y-4 mb-8">
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
              <h3 className="text-xs font-bold text-blue-600 uppercase tracking-widest mb-2">Game Overview</h3>
              <p className="text-slate-600 text-sm leading-relaxed">
                Predict <span className="text-red-600 font-bold">Red</span>, <span className="text-green-600 font-bold">Green</span>, or <span className="text-purple-600 font-bold">Violet</span>. Fast rounds (30-60s) with RNG-backed results and secure payouts.
              </p>
            </div>

            <div className="grid grid-cols-2 gap-3">
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                <p className="text-[10px] font-bold text-slate-400 uppercase">Special Rule</p>
                <p className="text-slate-700 text-xs font-medium">Violet pays higher multipliers.</p>
              </div>
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                <p className="text-[10px] font-bold text-slate-400 uppercase">Reliability</p>
                <p className="text-slate-700 text-xs font-medium">Independent random draws.</p>
              </div>
            </div>
          </div>
            ...
            <button
              ...
              className={`w-full py-4 px-6 rounded-2xl font-bold text-white shadow-xl transition-all transform active:scale-95 flex items-center justify-center gap-3 ${
                isRedirecting 
                  ...
                  : 'bg-[#24A1DE] hover:bg-[#1e87bb] hover:-translate-y-1 shadow-blue-200'
              }`}
            >
              {isRedirecting ? (
        ...
        <div className="bg-slate-50 py-4 px-8 border-t border-slate-100">
          <p className="text-[10px] text-slate-400 text-center leading-tight font-medium uppercase tracking-tighter">
            Play Responsibly • Official Channel
          </p>
        </div>
      </div>
    </div>
